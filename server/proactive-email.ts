...
// → recall user preferences → classifier → on important, route
...
const CLASSIFIER_MODEL = process.env.LLM_MODEL ?? "deepseek-v4-flash-free";
...
// Without dedup a duplicate delivery is exactly the
...
export async function classifyEmailImportance(
  ...
  try {
    ...
    console.error("[proactive] classifier LLM call failed", err);
  }
...
async function dispatchProactiveNotice(summary: string): Promise<void> {
  ...
  if (!chatId) {
    console.warn("[proactive] TELEGRAM_USER_ID not set; skipping dispatch");
    return;
  }

...
import type { IntegrationModule, ToolMap } from "./integrations/registry.js";
...
export async function authorizeToolkit(
  ...
  // Try new SDK link() first; fallback to initiate() for older SDK versions
  let conn: { redirectUrl?: string | null; id: string };
  try {
    conn = await composio.connectedAccounts.link(boopUserId(), authConfigId, {
      ...(existing.length > 0 ? { allowMultiple: true } : {}),
      ...(opts?.callbackUrl ? { callbackUrl: opts.callbackUrl } : {}),
      ...(opts?.alias ? { alias: opts.alias } : {}),
    });
  } catch {
    conn = await composio.connectedAccounts.initiate(boopUserId(), authConfigId, {
      ...(existing.length > 0 ? { allowMultiple: true } : {}),
      ...(opts?.callbackUrl ? { callbackUrl: opts.callbackUrl } : {}),
      ...(opts?.alias ? { alias: opts.alias } : {}),
    });
  }
  return { redirectUrl: conn.redirectUrl ?? null, connectionId: conn.id as string };
}
...
export function buildComposioIntegrationModule(slug: string): IntegrationModule {
  return {
    ...
    createTools: async (): Promise<ToolMap> => {
      const composio = getComposio();
      if (!composio) {
      ...
      const sdkTools = (await session.tools()) as unknown as ComposioToolDef[];
      const tools: ToolMap = {};
      for (const t of sdkTools) {
        if (!t.name) continue;
        tools[t.name] = tool(
          t.description ?? `Tool for ${slug}: ${t.name}`,
          (t.inputSchema ?? { type: "object", properties: {} }) as Record<string, unknown>,
          async (args) => {
            const result = await composio.tools.execute(t.name, {
              userId: boopUserId(),
              arguments: args,
              dangerouslySkipVersionCheck: true,
            });

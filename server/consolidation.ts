...
const ADVERSARY_MODEL = process.env.BOOP_ADVERSARY_MODEL ?? "deepseek-v4-flash-free";
const DEFAULT_MODEL = process.env.LLM_MODEL ?? "deepseek-v4-flash-free";
...
async function runLlm(
  ...
  try {
    ...
    buffer = result.text;
    usage = aggregateUsageFromResult(result.usage, model);
  } catch (err) {
...
export function startConsolidationLoop(intervalMs = 24 * 60 * 60 * 1000): () => void {
  // Fail any stale runs from previous sessions so the dashboard doesn't show
  // ghost "running" consolidations.
  failStaleRuns().catch(() => {});
...
async function failStaleRuns(): Promise<void> {
  try {
    const all = await convex.query(api.consolidation.listRuns, { limit: 50 });
    const stale = (Array.isArray(all) ? all : []).filter((r: any) => r.status === "running");
    for (const run of stale) {
      await convex.mutation(api.consolidation.updateRun, {
        runId: run.runId,
        status: "failed",
        notes: "Stale — killed on server boot",
      });
    }
    if (stale.length > 0) console.log(`[consolidation] failed ${stale.length} stale run(s)`);
  } catch (err) {
    console.warn("[consolidation] failStaleRuns error (non-fatal):", err);
  }
}
